...
router.get('/status', async (req, res) => {
  try {
    ...
    const companyNameSetting = await SystemSetting.findOne({ key: 'company_name' });
    const contactWebsiteSetting = await SystemSetting.findOne({ key: 'contact_website' });
    const contactEmailSetting = await SystemSetting.findOne({ key: 'contact_email' });
    const contactPhoneSetting = await SystemSetting.findOne({ key: 'contact_phone' });
    ...
    res.json({
      ...
      companyName: companyNameSetting ? companyNameSetting.value : 'PRASATEK SYSTEM SOLUTIONS',
      contactWebsite: contactWebsiteSetting ? contactWebsiteSetting.value : 'www.prasatek.lk',
      contactEmail: contactEmailSetting ? contactEmailSetting.value : 'info@prasatek.lk',
      contactPhone: contactPhoneSetting ? contactPhoneSetting.value : '0719323239'
    });
  } catch (error) {
